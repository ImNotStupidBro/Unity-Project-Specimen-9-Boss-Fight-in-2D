...
public class Game : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(debug == debugMode.ON)
        {
            ...
            {
                Instantiate(handWavePrefab, new Vector3(playerCharacter.transform.position.x, playerCharacter.transform.position.y - 2.2f, -2.1f), Quaternion.identity);
            }
            if(Input.GetKeyDown(KeyCode.T))
            {
                Instantiate(bodyPillarPrefab, new Vector3(player, 0, -2), Quaternion.identity);
            }
        }
