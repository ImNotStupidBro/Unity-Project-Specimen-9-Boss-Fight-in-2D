...
public class Game : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(debug == debugMode.ON)
        {
            ...
            {
                Instantiate(minionPrefab, new Vector3(10, 0, -2), Quaternion.identity);
            }
            if(Input.GetKeyDown(KeyCode.R))
            {
                Instantiate(handWavePrefab, new Vector3(playerCharacter.transform.position.x, playerCharacter.transform.position.y, -2), Quaternion.identity);
            }
            if(Input.GetKeyDown(KeyCode.T))
            {
                Instantiate(bodyPillarPrefab, new Vector3(playerCharacter.transform.position.x, -3.5f, -2), Quaternion.identity);
            }
        }
